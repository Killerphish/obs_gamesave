...
- **Setup weekend streams** – **Setup weekend** opens a dialog to create multiple YouTube broadcasts for a tournament weekend. Enter tournament name, your team name, and number of games; each game has its own tab with opponent, game time, and rink name. **Create all** creates one broadcast per game with title “*Your team* vs *Opponent*”, description “*Tournament* - *Rink*”, and the scheduled time. **Remember these settings** (when checked) saves tournament name, team name, game count, and per-game details to the OBS profile so the next time you open the dialog they are pre-filled.
